...
const fetchTasks = async (participationId: number, token: string): Promise<TasksData> => {
  const response = await fetch(`http://192.168.1.19:3000/tasks/get-tasks/${participationId}`, {
    headers: {
...
const deleteTaskApi = async (id: number, token: string) => {
  const response = await fetch(`http://192.168.1.19:3000/tasks/delete-task/${id}`, {
    method: "DELETE",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
